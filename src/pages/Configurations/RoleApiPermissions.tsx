    ...
    <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-4 sm:gap-6 animate-in fade-in duration-500 p-3 sm:p-5 md:p-6 w-full bg-background">
      ...
      <div className="w-full lg:w-72 flex-shrink-0 flex flex-col gap-3 lg:border-r border-slate-200 dark:border-zinc-800 lg:pr-4 pb-4 lg:pb-0 border-b lg:border-b-0 min-h-0">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">Select a role to configure API access.</p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <div>
          <h2 className="text-lg sm:text-xl font-bold tracking-tight text-foreground">Role API Permissions</h2>
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
             {selectedRole ? `Configuring CRUD API permissions for ${selectedRole.name}` : "Select a role from the list to begin."}
          </p>
          <div className="mt-2.5 p-2.5 bg-blue-50/80 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg flex items-start gap-2.5 text-xs">
            <div className="flex-shrink-0 mt-0.5">
              <svg className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {selectedRoleId ? (
          <div className="flex-1 min-h-0 flex flex-col gap-3 animate-in fade-in w-full mt-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
              ...
              {loadingRolePermissions && (
                <div className="flex justify-end text-xs text-muted-foreground items-center">
                  <Loader2 className="h-3.5 w-3.5 animate-spin text-blue-600 mr-1.5" /> Loading permissions...
                </div>
              )}
            </div>
            
            <div className={`flex-1 min-h-[260px] rounded-lg border bg-card shadow-xs overflow-hidden flex flex-col transition-opacity duration-200 ${loadingRolePermissions || saveMutation.isPending ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
              <div className="overflow-auto flex-1">
                <Table className="m-0 relative">
